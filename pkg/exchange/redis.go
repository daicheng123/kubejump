...
func newRedisManager(cfg Config) (*redisRoomManager, error) {
	...
	if cfg.UseSSL {
		...
		if cfg.SSLCert != "" && cfg.SSLKey != "" {
			...
			klog.Errorf("Load redis SSL cert: %s, key: %s", cfg.SSLCert, cfg.SSLKey)
		...
		if cfg.SSLCa != "" {
			...
			klog.Infof("Load redis SSL ca: %s", cfg.SSLCa)
	...
	if len(cfg.Clusters) > 0 {
		...
		sentinelConnFunc := func(network, addr string) (radix.Conn, error) {
			...
			if err != nil {
				klog.Errorf("Redis sentinelConnFunc dial err: %s", err)
				return nil, err
			}
			return conn, nil
		}
		serverConnFunc := func(network, addr string) (radix.Conn, error) {
			klog.Infof("sentinel pool server addr: %s", addr)
		...
		if err != nil {
			klog.Errorf("Redis sentinel client err: %s", err)
			return nil, err
		}
		connFunc = func(network, addr string) (radix.Conn, error) {
			// 选择一个master
	...
	if err != nil {
		klog.Errorf("Redis pubSub err: %s", err)
		return nil, err
	}
...
func (m *redisRoomManager) checkRoomExist(roomId string) bool {
	...
	if err != nil {
		klog.Errorf("Redis cache check room %s err: %s", roomId, err)
		return false
	}
	return countInt == 1
}
...
func (m *redisRoomManager) storeRoomId(roomId string) {
	...
	if err != nil {
		klog.Errorf("Redis Cache store room %s err: %s", roomId, err)
		return
	}
	klog.Errorf("Redis Cache store room %s success", roomId)
}
...
func (m *redisRoomManager) removeRoomId(roomId string) {
	...
	if err != nil {
		klog.Errorf("Redis cache remove room %s err: %s", roomId, err)
	} else {
		klog.Infof("Redis cache remove room %s success", roomId)
	}
	// 发布退出事件
	req := m.createRoomEventRequest(roomId, ExitEvent)
	_, err = m.sendRequest(&req)
	if err != nil {
		klog.Errorf("Redis cache publish room %s exit event err: %s", roomId, err)
	} else {
		klog.Infof("Redis cache publish room %s exit event success", roomId)
	}
}
...
func (m *redisRoomManager) run() {
	...
	for {
		...
		case req := <-m.reqChan:
			...
			case JoinEvent:
				...
				if room := m.remoteRoomCache.Get(req.RoomId); room != nil {
					klog.Error("Redis cache already create room %s", req.RoomId)
				...
				if err := m.publishRequest(req); err != nil {
					klog.Infof("Redis cache send request join room %s err: %s", req.RoomId, err)
					responseChan <- &subscribeResponse{
						Req:  req,
						room: nil,
						err:  err,
			...
			case ExitEvent:
				if err := m.publishRequest(req); err != nil {
					...
					klog.Errorf("Redis cache send request %s event %s err: %s", req.ReqId, req.Event, err)
					continue
				}
			...
			klog.Infof("Redis cache send event %s for room %s", req.Event, req.RoomId)

		case req := <-m.reqCancelChan:
			delete(requestsMap, req.ReqId)
			klog.Infof("Redis cache cancel request %s", req.ReqId)
		...
		case room := <-m.removeProxyRoomChan:
			...
			klog.Infof("Redis cache delete remote room %s", room.Id)
			m.remoteRoomCache.Delete(room)
			req := m.createRoomEventRequest(room.Id, LeaveEvent)
			if err := m.publishRequest(&req); err != nil {
				klog.Errorf("Redis cache send leave event for room %s err: %s", room.Id, err)
			} else {
				klog.Infof("Redis cache send leave event for room %s success", room.Id)
			}

		case redisMsg := <-m.subscribeEventsMsgCh:
			var req subscribeRequest
			if err := json.Unmarshal(redisMsg.Message, &req); err != nil {
				klog.Errorf("Redis cache unmarshal request msg err: %s", err)
				continue
			}
			...
			case resultsChannel:
				...
				case JoinSuccessEvent:
					...
					if !ok {
						klog.Infof("Redis cache ignore not self result request %s", req.ReqId)
						continue
					}
					klog.Infof("Redis cache request %s receive result", req.ReqId)
					// 请求结束，移除缓存, 返回请求的结果
					delete(requestsMap, req.ReqId)
					...
					if err != nil {
						klog.Errorf("Redis cache request %s create redis conn err: %s", req.ReqId, err)
						res.err = err
						responseChan <- &res
						continue
					}
					...
					if err = pubSub.Subscribe(redisMsgCh, readChannel); err != nil {
						_ = pubSub.Close()
						klog.Errorf("Redis cache request %s subscribe channel err: %s", req.ReqId, err)
						res.err = err
						responseChan <- &res
						continue
					}
					...
					klog.Infof("Redis cache request %s finished", req.ReqId)
				default:
					klog.Infof("Result channel receive unhandled event %s", req.Event)
				}
				...
				case JoinEvent:
					...
					*/
					...
					if _, ok := requestsMap[req.ReqId]; ok {
						klog.Infof("Redis cache ignore self request %s", req.ReqId)
						continue
					}
					...
					if srv, ok := redisUserCons[req.RoomId]; ok {
						klog.Infof("Redis cache already create redis con for room %s", req.RoomId)
						if err := m.publishRequest(&successReq); err != nil {
							klog.Errorf("Redis cache reply request %s join event err %s", req.ReqId, err)
						} else {
							klog.Infof("Redis cache reply request %s join event", req.ReqId)
							//  统计一下 req的 count
					...
					if r := m.localRoomCache.Get(req.RoomId); r != nil {
						redisCon, err := m.connFunc("", "")
						if err != nil {
							klog.Errorf("Redis cache create redis conn for request %s err %s", req.ReqId, err)
							continue
						}
						pubSub := radix.PubSub(redisCon)
						...
						if err = pubSub.Subscribe(subMsgCh, readChannel); err != nil {
							_ = pubSub.Close()
							klog.Errorf("Redis cache create pubSub conn for request %s err: %s", req.ReqId, err)
							continue
						}
					...
					klog.Infof("The current KoKo node has no session room %s", req.RoomId)
					// 非本节点 koko 创建的session
				case LeaveEvent:
					if srv, ok := redisUserCons[req.RoomId]; ok {
						srv.addSubscribeCount(-1)
						klog.Infof("Event channel receive room %s leave event", req.RoomId)
					}

				case ExitEvent:
					if room := m.remoteRoomCache.Get(req.RoomId); room != nil {
						klog.Infof("Event channel receive room %s exit", req.RoomId)
						m.remoteRoomCache.Delete(room)
					}
				default:
					klog.Infof("Event channel receive unhandled event %s: %v", req.Event, req)
				}
...
func (m *redisRoomManager) getRemoteSessionRoom(roomId string) *Room {
	klog.Infof("Waiting subscribe remote room %s result", roomId)

	req := m.createRoomEventRequest(roomId, JoinEvent)
	res, err := m.sendJoinRequest(&req)
	if err != nil {
		klog.Errorf("get remote session room err: %s", err)
		return nil
	}
	return res.room
}
